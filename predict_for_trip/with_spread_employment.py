...
def walk_forward(df: pd.DataFrame, pair: str):
    ...
    # 4. 欠損処理
    df = df.dropna(subset=["log_ret", "log_ret_next", "sent_mean", "post_count"]).reset_index(drop=True)

    # 特徴量行列
    features = df[["log_ret", "sent_mean", "post_count", "spread_chg", "PAYEMS_chg"]].to_numpy(dtype=np.float32)
...
def main():
    for pair in PAIRS:
        path = os.path.join(DATA_DIR, f"{pair}_with_bluesky_cbank_emp.csv")
