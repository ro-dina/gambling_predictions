...
def load_fx_bluesky(base: str, target: str, date_start: str, date_end: str) -> pd.DataFrame:
    ...
    """
    pair = f"{base}{target}"
    csv_path = f"data/fx_bluesky/{pair}_with_bluesky_cbank.csv"
...
def run_pair_walk_forward(base: str, target: str, date_start: str, date_end: str,
    ...
    # 為替レートとセンチメントだけを必須列とし、中央銀行データは NaN を許容する
    subset_cols = ["value", "sent_mean"]
    df = df.dropna(subset=subset_cols).reset_index(drop=True)

        # return系列を作成（log return）
    df["ret"] = np.log(df["value"]).diff()
    df = df.dropna(subset=["ret"]).reset_index(drop=True)

    # ① 政策金利列を前方埋め / 後方埋めし、それでも残る NaN は 0 で埋める
    ...
    # ② 為替レート・センチメント・ret のみを必須列とし、政策金利列は NaN を許容する
    subset_cols = ["value", "sent_mean", "ret"]
    df = df.dropna(subset=subset_cols).reset_index(drop=True)
    ...
    if n == 0:
        raise RuntimeError(
            f"{pair}: return系列が0件です。value 列や date の範囲に問題がないか、"
            f"data/fx_bluesky/{pair}_with_bluesky.csv の中身を確認してください。"
        )

    # BlueSky由来の特徴は、まずは sent_mean のみを利用する
    sent_mean = df["sent_mean"].to_numpy(dtype=np.float64)

    # 中央銀行の政策金利（列が存在しない場合は 0 配列で代用）
    length = len(df)

    def policy_or_zeros(col_name: str) -> np.ndarray:
        if col_name in df.columns:
            return df[col_name].to_numpy(dtype=np.float64)
        else:
            return np.zeros(length, dtype=np.float64)

    jp_policy = policy_or_zeros("JP_POLICY")

    if pair == "USDJPY":
        foreign_policy = policy_or_zeros("US_POLICY")
    elif pair == "EURJPY":
        foreign_policy = policy_or_zeros("EU_POLICY")
    elif pair == "GBPJPY":
        foreign_policy = policy_or_zeros("UK_POLICY")
    else:
        raise RuntimeError(f"未知の通貨ペア: {pair}")

    # 通貨ペアごとに「外国 - 日本」の金利スプレッドを作る
    spread = foreign_policy - jp_policy

    # 特徴量行列: [log_return, sent_mean, rate_spread]
    features_raw = np.stack(
        [rets, sent_mean, spread],
        axis=1,
    )  # (n, 3)

    n_features = features_raw.shape[1]

    # ここでは n > 0 であることのみ保証されている（上のガードでチェック済み）
    n = len(rets)
